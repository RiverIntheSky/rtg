...
SharedAtomicCounterBuffer AtomicCounterBuffer::create(uint32_t value, size_t count)
{
...
{
    GLOW_RUNTIME_ASSERT(sCurrentBuffer == this, "Currently bound UB does NOT match represented buffer", return false);
    return true;
}
...
void AtomicCounterBuffer::BoundAtomicCounterBuffer::setCounters(uint32_t value, size_t count, GLenum usage)
{
    setCounters(std::vector<uint32_t>(count, value));
}

uint32_t AtomicCounterBuffer::BoundAtomicCounterBuffer::getCounter(size_t pos)
{
    if (!isCurrent())
        return 0u;
...
size_t AtomicCounterBuffer::BoundAtomicCounterBuffer::getCounterSize() const
{
