...
private:
    /// Offset to verify
    std::map<std::string, size_t> mVerificationOffsets;

public: // getter
    /// Gets the currently bound UniformBuffer (nullptr if none)
    static BoundUniformBuffer* getCurrentBuffer();

    std::map<std::string, size_t> const& getVerificationOffsets() const { return mVerificationOffsets; }
public:
...
public:
    ...
    void addVerification(size_t offset, std::string const& nameInShader);
