...
void ArrayBuffer::defineAttribute(const ArrayBufferAttribute &a)
{
    ...
    if (a.fixedStride > 0)
    {
        ...
        {
            error() << "Attribute requires fixed stride of " << a.fixedStride << " but stride is already set to " << mStride;
...
{
    GLOW_RUNTIME_ASSERT(sCurrentBuffer == this, "Currently bound AB does NOT match represented buffer", return false);
    return true;
}
