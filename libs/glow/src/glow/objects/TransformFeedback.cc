...
void TransformFeedback::BoundTransformFeedback::setFeedbackBuffer(const SharedBuffer &feedbackBuffer)
{
    ...
    GLOW_RUNTIME_ASSERT(!feedback->isRecording(), "Cannot set feedback buffer while recoding", return );
...
void TransformFeedback::BoundTransformFeedback::begin(GLenum primitiveMode)
{
    ...
    GLOW_RUNTIME_ASSERT(!feedback->isRecording(), "Cannot call begin when Feedback is still running", return );
...
bool TransformFeedback::BoundTransformFeedback::isCurrent() const
{
    GLOW_RUNTIME_ASSERT(sCurrentFeedback == this,
                        "Currently bound TransformFeedback does NOT match represented feedback", return false);
    return true;
}
